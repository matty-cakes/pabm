...
const create_step_entry = (starting_current_largest, compared_to, was_replaced) => ({
...
const selection_sort = (items) => {
  ...
  while(items.length) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      step[count].entries.push(create_step_entry(
        starting_current_largest,
        item,
        was_replaced,
      ))
    ...
    step[count]["ending_items"] = cheap_deep_copy(items)
    step[count]["ending_sorted_items"] = cheap_deep_copy(sorted_items)
    steps.push(step)
    count++
